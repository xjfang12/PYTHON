    ...
    if response.status_code == 200:
        ...
        with open(path, 'wb') as file:
            for data in response.iter_content(chunk_size=chunk_size):
                file.write(data)
                size += len(data)
                print('\r'+'[下载进度]:{}{}%'.format(">"*int(size * 50 / content_size), float(size/content_size * 100)),end = ' ')
                end = time.time()
                print('\n' + '视频下载完成!用时: {}'.format(end -start))

def The_url(page):
    url = 'http://api.vc.bilibili.com/board/v1/ranking/top?page_size=10&next_offset=' + str(
        page) + '&tag=%E4%BB%8A%E6%97%A5%E7%83%AD%E9%97%A8&platform=pc'
